...
TEST(And, OneOperatorFunctorArg)
{
    using namespace operatorFunctors;

    const auto func = []<bool Value, bool... VRest>(const auto& execFunc)
    {
    ...
    func.template operator()<false, true>(func);
}

TEST(And, TwoOperatorFunctorArg)
{
    using namespace operatorFunctors;
